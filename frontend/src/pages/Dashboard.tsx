...
export const Dashboard = () => {
    ...
    const weightHistory = useMemo(() => {
        ...
        const lastWeekData = measurements.filter(m => 
        ...
        return sortedData.map((item, index, array) => {
            ...
            const prev1 = array[index - 1] ? parseFloat(array[index - 1].value) : null;
            const prev2 = array[index - 2] ? parseFloat(array[index - 2].value) : null;

            let sum = currentValue;
            let count = 1;

            if (prev1 !== null) { sum += prev1; count++; }
            if (prev2 !== null) { sum += prev2; count++; }
    ...
    const totalWeeklySets = exerciseTypeSetCount.reduce((sum, item) => sum + item.count, 0);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2">
                    <CardHeader>
                        ...
                        <CardDescription>Your weight vs. weekly moving average.</CardDescription>
                    </CardHeader>
                            ...
                            <div className="flex justify-between items-center">
                                {WEEKLY_ACTIVITY.map((item, index) => (
                                    <div key={index} className="flex flex-col items-center gap-2">
                                        ...
                                        {/* The Label */}
                                        <span className={`text-xs font-semibold ${item.active ? 'text-gray-800' : 'text-gray-400'}`}>
                                            {item.day.charAt(0)}
                                        </span>
                                    </div>
                                ))}
                ...
                <Card className="lg:col-span-3">
                    <CardHeader>
                        ...
                        <CardDescription>Total sets performed per muscle group this week.</CardDescription>
                    </CardHeader>
